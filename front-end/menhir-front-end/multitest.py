...
TESTROOT = os.path.join('..' , 'testsuite-python-lib')
...
class TestHarness:
    ...
    def make_executable(self, version, forceMake=False):
        ...
        if forceMake or not os.path.isfile(self.ver_front_end):
            print("Going to make front-end for version %s..." % version)

            retcode = TestHarness.shell_run('make clean; make PYVER=%s' % version)
    ...
    def test_one_file(self, root, filename):
        ...
        toExec = '%s < %s > %s 2>&1' \
                 % (self.ver_front_end, filepath, TestHarness.ERROR_LOG)
