...
module pyparse::RunTransformation
...
import pyparse::extract::PyEBNF2BGF;  // preparePythonGrammar

import pyparse::extract::Gram2Bison;
import pyparse::extract::Gram2Menhir;
...
{ 
	...
	if (endsWith(outfile.file, BISON_GAMMAR_DOT_SUFFIX))
		appendToFile(outfile, ["<s>\n" | s <- strBison(pyg)]);
	else if (endsWith(outfile.file, MENHIR_GAMMAR_DOT_SUFFIX))
		appendToFile(outfile, ["<s>\n" | s <- strMenhir(pyg)]);
	return pyg;
}
...
public void generateBisonTransform(loc infolder, str infile,  loc outfolder)
{
	...
	assert ext notin [BISON_GAMMAR_SUFFIX, GLUE_FILE_SUFFIX] : "I will need to overwrite these";
	...
	pyparse::Gram2Bison::translateFile(infolder+infile, outfolder+outfile, outfolder+glufile);
...
// Test cases: a small one -
void gt2()
{
	generateBisonTransform(res(), "test2.ebnf");
}

/// ... and a big one:
void gt()
{
	generateBisonTransform(res(), "2.7.2.txt", res());
}

void t(str ver) {
    runTransforms(pyg("bisonified-parsers")+"<ver>.y", 
                  [wip("manual-glue")+"<ver>-manual.glue"], 
                  wip("<ver>.y"), 
                  0);
}
void t() = t("3.1");
