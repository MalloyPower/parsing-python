// Generated (bisonified) from 1.4.txt on 2016-11-23 at 16:45:41 +0000
...
%token DEDENT DEF DEL DOT DOUBLESTAR ELIF ELSE ENDMARKER EQEQUAL EQUAL EXCEPT
%token EXEC FINALLY FOR FROM GLOBAL GREATER GREATEREQUAL GRLT IF IMPORT
%token IN INDENT IS LAMBDA LBRACE LEFTSHIFT LESS LESSEQUAL LPAR LSQB MINUS
%token NAME NEWLINE NOT NOTEQUAL NUMBER OR PASS PERCENT PLUS PRINT RAISE
%token RBRACE RETURN RIGHTSHIFT RPAR RSQB SEMI SLASH STAR STRING TILDE TRY
%token WHILE
...
raise_stmt // Used in: flow_stmt
	: RAISE test COMMA test opt_COMMA_test
	| RAISE test
	;
opt_COMMA_test // Used in: raise_stmt, exec_stmt, except_clause
	: COMMA test
	| %empty
	;
...
plus_stmt // Used in: suite, plus_stmt
	...
	| stmt
	;
test // Used in: opt_EQUAL_test, print_stmt, star_test_COMMA, raise_stmt, opt_COMMA_test, exec_stmt, if_stmt, star_ELIF, while_stmt, except_clause, lambdef, subscript, opt_test_only, sliceop, testlist, star_COMMA_test, dictmaker, star_test_COLON_test, argument
